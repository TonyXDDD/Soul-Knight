...
func _physics_process(delta: float):
	...
	if movement_timeout_timer > 0:
		...
		facing_knight()  # Ensure sprite faces the knight
...
func attack_player():
	if not is_attacking:  # Check if not already attacking
		...
		if is_player_in_damage_range():
			player.take_damage(50)  # Apply damage only if player is in damage range
...
func get_player_in_group() -> Node2D:
	return get_tree().get_nodes_in_group("Player")[0]  # Assuming only one player is in the group

# Function to ensure the enemy sprite faces the knight
func facing_knight():
	if player.position.x > position.x:
		animated_sprite_2d.flip_h = false  # Face right
	else:

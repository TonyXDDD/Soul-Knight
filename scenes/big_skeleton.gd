...
@export var speed: float = 110.0  # Speed of the enemy
...
# HP reference
var health: int = 100  # Health variable for the enemy
...
var attack_timer: float = 3.5  # Timer to track the cooldown
var movement_timeout: float = 3.5  # Time to remain idle after attacking
...
# Mouse click cooldown variables
var left_click_cooldown: float = 0.3  # Cooldown for left click damage
var right_click_cooldown: float = 0.6  # Cooldown for right click damage
var left_click_timer: float = 0.0  # Timer for left click cooldown
var right_click_timer: float = 0.0  # Timer for right click cooldown
...
func _physics_process(delta: float):
	...
	# Update left and right click cooldown timers
	...
	if is_player_very_close():
		if Input.is_action_just_pressed("left_mouse_click") and left_click_timer <= 0:
			health -= 5
			flash_effect()
			left_click_timer = left_click_cooldown  # Start left click cooldown
		elif Input.is_action_just_pressed("right_mouse_click") and right_click_timer <= 0:
			health -= 10
			flash_effect()
			right_click_timer = right_click_cooldown  # Start right click cooldown

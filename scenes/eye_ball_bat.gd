...
var attack_cooldown: float = 1.0  # Cooldown period between attacks
var attack_timer: float = 0.5  # Timer to track the cooldown
var movement_timeout: float = 0.5  # Time to remain idle after attacking
...
func _physics_process(delta: float):
	...
	if is_player_very_close():
		if Input.is_action_just_pressed("left_mouse_click") and left_mouse_timer <= 0:
			health -= 20  # Decrease health by 10 for left click
			flash_effect()
			...
			health -= 30  # Decrease health by 15 for right click
			flash_effect()
